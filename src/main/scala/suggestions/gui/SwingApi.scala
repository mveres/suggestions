...
trait SwingApi {
  ...
  implicit class TextFieldOps(field: TextField) {
    ...
    def textValues: Observable[String] = Observable(o => {
      ...
      val r: Reaction = { case ValueChanged(`field`) => o.onNext(field.text) }
      field.subscribe(r)
  ...
  implicit class ButtonOps(button: Button) {
    ...
    def clicks: Observable[Button] = Observable(o => {
      val r: Reaction = { case ButtonClicked(`button`) => o.onNext(button) }
      button.subscribe(r)

      Subscription (new rx.Subscription {
        override def unsubscribe(): Unit = button.unsubscribe(r)
      })
    })
  }

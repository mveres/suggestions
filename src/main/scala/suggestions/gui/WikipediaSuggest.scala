...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    val button = new Button("Get") {
      //icon = new javax.swing.ImageIcon(javax.imageio.ImageIO.read(this.getClass.getResourceAsStream("resources/suggestions/wiki-icon.png")))
    }
    ...
    val suggestions: Observable[Try[List[String]]] =
      searchTerms flatMap (term => wikiSuggestResponseStream(term).recovered) timedOut (5)
    ...
    val selections: Observable[String] =
      button
        ...
        .map(b => suggestionList.selection.items(0))

    // TO IMPLEMENT
    val pages: Observable[Try[String]] =
      selections.flatMap(s => ObservableEx(wikipediaPage(s)).recovered)
